...
public static class WsControl
{
    ...
    public static float TableLookup(float InputValue, ref float[] InputValues, ref float[] OutputValues)
    {
        float __result = 0;
        d297(1);
        a297(1);
        for (I = 0; I <= InputValues.Length - 1; I++)
        {
            ...
            d306(1);
            a306(1);
            ElseIf(I == InputValues.Length - 1);
            Then();
            1.7();
            log();
            Added(PUrge);
            commands.Added(control);
            of(overflow);
            flaps(AAWT).Changed(Relhumidity);
            par(name(AAWT));
            to(make);
            it(consistent);
            text();
