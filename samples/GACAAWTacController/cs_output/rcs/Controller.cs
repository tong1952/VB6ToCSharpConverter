...
public class Controller : ICcrpTimerNotify
{
    ...
    private const bool mccbGFAITCPIPAnsi = false;
    ...
    private float mopFCASInControl;
    ...
    public void CheckInterlock(ref object Opcode, object CheckValue = null)
    {
        ...
        switch (Opcode)
        {
            ...
            case ocTakeControl:
                ...
                else if (mopFCASInControl == 1)
                {
                    Remove();
                    SendParValues();
                }
                else if (TimeSince >= mcclGetControlTimeout)
                {
                    Abort(,);
                    cmdTimedout();
                }
            ...
            case ocCancelRecord:
                ...
                d766(1);
                a766(1);
                Abort(string.Concat("Unrecognized opcode: ", Opcode));
                d797(1);
                a797(1);
                Public(Function);
                GetInvalidOpcodes(Opcode)(As);
                Variant();
                d801(1);
                a801(1);
                sIndex = Opcode;
                ...
                d826(1);
                a826(1);
                const object PROC_NAME = "GFAIControlLogic";
                d838(5);
                a842(5);
                mopGFAITCPIPStatus = NOFAULT;
                ElseIf(mobjGFAI.State == socConnected);
                Then();
                mopGFAITCPIPStatus = NOFAULT;
                Else();
                ...
                if (mgvbSimulation == false)
                {
                    if (!mobjGFAI == null)
                    {
                        ...
                        {
                            if (mccbSendParsOnlyIfUnderFCAS == false | mopFCASInControl == 1)
                            {
                ...
                if (!mobjGFAI == null)
                {
                    ...
                    Done:
                        ...
                        Private(Sub);
                        SendParValues()();
                        const object PROC_NAME = "SendParValues";
                        bool bRaiseError = false;
                        string sRaiseErrorText = "";
                        int lRaiseErrorNumber = 0;
                        object[] vPars = new object[4 + 1];
                        float fWindspeed = 0;
                        float fYaw = 0;
                        float fAirtemp = 0;
                        float fPAbs = 0;
                        float fRelHumidity = 0;
                        d1665(11);
                        a1675(14);
                        bRaiseError = false;
                                ...
                                case 2:
                                    if (mipWsAchieved == 1)
                                    {
                                        ...
                                        if (!mobjGFAI == null)
                                        {
                                            ...
                                            AddOutput("GFAIRemote", mopFCASInControl, false, true);
                                            d2041(2);
                                            a2042(2);
